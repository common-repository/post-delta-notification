    ...
    class PostDeltaNotification {
        ...
        function pdn_PrintOptionPage() {
            ...
            if ( isset( $_POST['update_pdn_Settings'] ) ) {
                ...
                if ( isset($_POST['PostDeltaNotification_IDsToExclude']) ) {
                    $devOptions['exclude_ids'] = apply_filters( 'content_save_pre', $_POST['PostDeltaNotification_IDsToExclude'] );
                }
            ...
            if ( isset( $updated ) && $updated ) {
                ...
                if ( count( $optionsErrors ) > 0 ) {
                    echo "\t<p>There are errors in your configuration. Odds are good that emails will not be sent successfully.</p>\n";
                    echo "\t<ul>\n";
                ...
                echo "<p><strong>Settings failed to update.</strong></p>\n";
                if ( count( $optionsErrors ) > 0 ) {
                    echo "\t<p>There are errors in your configuration. Odds are good that emails will not be sent successfully.</p>\n";
                    echo "\t<ul>\n";
                    foreach ( $optionErrors as $error ) {
...
<h2>Post Delta Notification Settings</h2><?php
            echo "<h3 style='margin-bottom: -5px;'>Email From Address</h3>\n";
            echo "\t<p><input id='PostDeltaNotification_fromAddress' type='text' name='PostDeltaNotification_fromAddress' value='" . $devOptions['fromAddress'] . "' style='width: 250px;'><br>\n";
            echo "\tEmails sent to users will use this as the \"from\" address. If blank, the default WP from address is used.</p>\n";
            echo "<h3 style='margin-bottom: -5px;'>Email From Name</h3>\n";
            echo "\t<p><input id='PostDeltaNotification_fromName' type='text' name='PostDeltaNotification_fromName' value='" . $devOptions['fromName'] . "' style='width: 250px;'><br>\n";
            echo "\tEmails sent to users will use this as the \"from\" display name. If blank, the default WP from name is used.</p>\n";
            echo "<h3 style='margin-bottom: -5px;'>Per Email Recipient Limit</h3>\n";
            echo "\t<p><input id='PostDeltaNotification_recLimit' type='text' name='PostDeltaNotification_recLimit' value='" . $devOptions['limitToXRecipients'] . "' style='width: 150px;'><br>\n";
            echo "\tSome providers limit the number of recipients that can be on a single email. You can enter your limit here or set it to zero (\"0\") for unlimited. If the number of intended recipients is higher than this, he system will split the number across multiple messages.</p>\n";
            echo "<h3 style='margin-bottom: -5px;'>Subscribe Button Position</h3>\n";
            echo "\t<p><input id='PostDeltaNotification_footerAlignment_left' type='radio' name='PostDeltaNotification_footerAlignment' value='left'" . ( $devOptions['footerAlignment'] == 'left' ? " checked" : "" ) . ">Left</input>\n";
            echo "\t<input id='PostDeltaNotification_footerAlignment_center' type='radio' name='PostDeltaNotification_footerAlignment' value='center'" . ( $devOptions['footerAlignment'] == 'center' ? " checked" : "" ) . ">Center</input>\n";
            echo "\t<input id='PostDeltaNotification_footerAlignment_right' type='radio' name='PostDeltaNotification_footerAlignment' value='right'" . ( $devOptions['footerAlignment'] == 'right' ? " checked" : "" ) . ">Right</input><br>\n";
            echo "\tThe subscribe and unsubscribe buttons will appear at the bottom of each post and will be aligned by this option.</p>\n";
            echo "\t<h3 class=''>Page, post IDs to exclude</h3>\n";
            echo "\t<p class=''><input type='text' name='PostDeltaNotification_IDsToExclude' style='width:90%;' value='" . apply_filters( 'format_to_edit', $devOptions['exclude_ids'] ). "'><br />\n";
            echo "\tThis is the comma-separated list of IDs that will not show the subscribe/unsubscribe button.</p>\n";
...
?></form>
        ...
        function pdn_SendEmail( $post_id, $newPost, $oldPost ) {
            ...
            $message = "<p>A blog post on \"" . get_bloginfo( 'name' ) . "\" has been updated. Click the post title below to see the new content.</p>\n";
            $message .= "<p><a href='" . get_permalink( $newPost->ID ) . "' target=_blank>" . apply_filters( 'the_title', $newPost->post_title ) . "</a></p>\n";
            $message .=  "<p>On this page, you can unsubscribe from future email updates.</p>\n";
        ...
        function pdn_ContentInsertion( $content ) {
            ...
            //Only show buttons if post id is not excluded
            ...
            if ( "https://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'] == get_permalink() && !is_bool( $postType ) ) {
...
jQuery( "#PostDeltaNotification_SubButton" ).click(function(e){
        ...
        function pdn_ParseRequest( $wp ) {
            if ( array_key_exists( 'postdeltanotification', $_GET ) ) {
                ...
                //echo "<!-- operation: " . $operation . ". post: " . $post . ". email: " . $email . ". -->\n";
